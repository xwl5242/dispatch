...
import com.frames.util.Const;
import com.frames.util.PageData;
import com.frames.util.Tools;
...
public class Page {
	...
	public String getPageStr() {
		...
		if(totalResult>0){
			sb.append("	<ul style=' width: 781.500px;height: 30px;'>\n");
			if(currentPage==1){
			...
			for(int i=1;i<10;i++){
				sb.append("	<option value='"+(i*10)+"' "+getOptionSelected(showCount,(i*10))+">"+(i*10)+"</option>\n");
			}
			sb.append("	<option value='99' "+getOptionSelected(showCount,99)+">99</option>\n");
			sb.append("	</select>\n");
			sb.append("	</li>\n");
			sb.append("</ul>\n");
	...
	public String getOptionSelected(int showCount,int num){
		String selceted="";
		if(showCount==num){
			selceted="selected = \"selected\"";
		}
		return selceted;
	}
	...
	/**
	 * 获得描述：String  sqlPage. <BR>
	 * @param sqlPage the sqlPage to set
	 */
