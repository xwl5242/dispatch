...
<body id='body'>
	 	...
	 	<form id="roleUpdateform">
		  	<table cellpadding="0" cellspacing="0" class="tbl_search_bg" width="100%">
				...
				<tr style="text-align: right;">
					<td colspan="4"> 
					   <a id="btn"  onclick='updateRoleData()' class="easyui-linkbutton" data-options="iconCls:'icon-save'">保存</a>&nbsp;
	...
	function updateRoleData(){
		if(!$("#roleUpdateform").form('validate')){
			return false;
		}
		$.messager.progress();//打开进度条
		$.ajax({ 
            type: "post", 
            url: "<%= request.getContextPath()%>/role/updateRole.do?cmd=updateRole",
            data:$('#roleUpdateform').serialize() ,//form表单序列化
            dataType: "json", 
	...
	$.ajax({ 
        type: "post", 
        url: "<%= request.getContextPath()%>/role/findRoleById.do?cmd=findRoleById",
        data: {"RoleId":selRows[0].ID},//form表单序列化
        dataType: "json", 
        success: function(data){
        	$('#roleUpdateform').form('load',data);
        	$("#RoleId").val(data.id);
        },
        error:function(){ 
        	  $.messager.alert("警告","系统异常！",'warning'); 
