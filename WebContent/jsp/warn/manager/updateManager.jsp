...
<html>
<head>
<title>修改预警管理信息</title> 
<jsp:include page="/jsp/header.jsp" />
</head>
<body id='body'>
	 <div id="ManagerUpdatePanel" class="easyui-panel"  style="width:100%;" style="height:100%;" data-options="collapsible:false">
	 		...
	 		<input id="id" type="hidden" />
		  	<table cellpadding="0" cellspacing="0" class="tbl_search_bg" width="100%">
				...
				<tr style="text-align: right;">
					<td colspan="4"> 
					   <a id="btn"  href='javascript:updateManager()' class="easyui-linkbutton" data-options="iconCls:'icon-save'">保存</a>&nbsp;
					   <a  onclick='updateManagerClose()' class="easyui-linkbutton" data-options="iconCls:'icon-cancel'">关闭</a>  &nbsp;
					</td> 
				</tr>
			</table>
		</form>  
	...
	function updateManagerClose(){ 
		$.messager.confirm('确认','您确认想要关闭吗？',function(r){    
		    if (r){ 
		    	$('#updateManagerDd').dialog("close");
			}
	 });
	} 
	...
	function updateManager(){
		var json=JSON.stringify($("#ManagerUpdateform").serializeObject());
		$.messager.progress();//打开进度条
		$.ajax({ 
            type: "post", 
            url: "<%= request.getContextPath()%>/manager/updateManager.do?cmd=updateManager",
            data: {json:json,id:$("#id").val(),"type":$("input[name='dianType']").val()},//form表单序列化
            dataType: "json", 
            success: function(data){
            	 $.messager.progress('close');//关闭进度条
            	if(data.flag){ 
		    		 $('#ManagerlistGrid').datagrid('reload');
				     $.messager.alert("提示",data.messager,'info',updateManagerClose()); 
		    	}else{
	 ...
	 $(function(){
		$.ajax({ 
	        type: "post", 
	        url: "<%= request.getContextPath()%>/manager/findManagerById.do?cmd=findManagerById",
	        data: {"id":selRows[0].ID,"type":selRows[0].TYPE},//form表单序列化
	        dataType: "json", 
	        success: function(data){
	        	$('#ManagerUpdateform').form('load',data);
	        	$("#id").val(data.id);
	        	if(data.anaWarnFun==null || data.anaWarnFun=="" || data.anaWarnFun=="null"){
	        		$("#ana").hide();
	        		$("#onoff").show();
	        	}
	        	if(data.onoffWarnFun==null || data.onoffWarnFun=="" || data.onoffWarnFun=="null"){
	        		$("#ana").show();
	        		$("#onoff").hide();        		
	        	}
	        	$("#unitName").textbox('setText',data.unitName);
	        	$("#collectName").textbox('setText',data.collName);
	        	$("#temp-unitId").val(data.unitId);
	        	$("#collId").val(data.collId);
	        	$("#collUnit").val(data.unitId);
	        	$("#dianType").combobox('setValue',data.type);
	        },
	        error:function(){ 
	        	  $.messager.alert("警告","系统异常！",'warning'); 
	        }
